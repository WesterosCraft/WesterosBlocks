...
public class ArrowSlitBlockExport extends ModelExport {
    ...
    public void generateBlockStateModels() {
        ...
        // Process each state (typically just one for arrow slits)
        for (ModBlockStateRecord state : def.states) {
            String stateID = state.stateID != null ? state.stateID : "";
            boolean isCustom = state.isCustomModel();

            // Get the base model path
            String modelPath = String.format("%stest_arrow_slit/%s",
                    isCustom ? CUSTOM_PATH : GENERATED_PATH,
                    stateID.isEmpty() ? "test_arrow_slit_single" : "test_arrow_slit_" + stateID);
            ...
            for (int rotation = 0; rotation < 360; rotation += 90) {
                ...
                // Add variant with facing condition
                String condition = String.format("facing=%s", getFacingDirection(rotation));
    ...
    public static void generateItemModels(ItemModelGenerator itemModelGenerator, Block block, ModBlock blockDefinition) {
        ...
        generateBlockBasedItemModel(itemModelGenerator, block, blockDefinition, "test_arrow_slit_single");
    }
} 